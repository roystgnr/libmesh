...
protected:
  ...
  void test_sphere(unsigned int n_refinements, const ElemType type = QUAD9)
  {
    ...
    auto check_radii = [&boundary_mesh, radius, type](Real radius_tol) {
      ...
      std::cerr << "mesh.n_elem() = " << boundary_mesh.n_elem() << std::endl;
    ...
    auto verify_equispaced_midnodes = [&boundary_mesh]()
    {
      ...
      for (auto & elem : boundary_mesh.element_ptr_range())
        {
          ...
          if (elem->type() == QUAD9)
            {
              auto c08 = elem->point(8) - elem->point(0);
              auto c18 = elem->point(8) - elem->point(1);
              auto c28 = elem->point(8) - elem->point(2);
              auto c38 = elem->point(8) - elem->point(3);
              libmesh_assert_less
                (std::abs(c08.norm_sq() - c18.norm_sq()), TOLERANCE*TOLERANCE);
              libmesh_assert_less
                (std::abs(c08.norm_sq() - c28.norm_sq()), TOLERANCE*TOLERANCE);
              libmesh_assert_less
                (std::abs(c08.norm_sq() - c38.norm_sq()), TOLERANCE*TOLERANCE);
            }
        }
    };
    ...
    // still have error ... of roughly the same order???
    /*
    const Real max_rbb_rad_error =
      radius * 0.1 / (1 << (4*n_refinements));
    ...
    */
    ...
    // have error, but our quadrature error looks something like Ch^6
    // with a much smaller C. ???
    const Real max_rbb_vol_error = 0;
      //radius * 1e-3 / (1 << (6*n_refinements));
    LIBMESH_ASSERT_FP_EQUAL(MeshTools::volume(boundary_mesh),
